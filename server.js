...
const app = express();
// Render ke liye environment port zaroori hai
...
// Sahi Folder Structure: Static files 'school.html' folder ke andar hain
...
// --- RAZORPAY SETUP (Updated with your Keys) ---
// Yahan hum Render ke environment variables se keys utha rahe hain
const razorpay = new Razorpay({
    key_id: process.env.RAZORPAY_KEY_ID || 'rzp_test_SNWAKgO88lL2lq', 
    key_secret: process.env.RAZORPAY_KEY_SECRET || 'n80kbgdXcTUIAPVMN5b00310'
});
...
app.post('/create-order', async (req, res) => {
    try {
        const options = { 
            amount: req.body.amount * 100, // Razorpay paise ko paise mein leta hai
            currency: "INR", 
...
app.post('/submit-admission', async (req, res) => {
    try { 
        await new Admission(req.body).save(); 
        res.send('<h2>Submitted! <a href="/">Go Back</a></h2>'); 
...
app.post('/submit-contact', async (req, res) => {
    try { 
        ...
        res.send('<h2>Sent! <a href="/">Go Back</a></h2>'); 
    } catch (e) { res.status(500).send('Error'); }
});
